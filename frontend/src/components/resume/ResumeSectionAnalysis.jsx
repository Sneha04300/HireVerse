...
function SectionCard({ section }) {
  ...
  return (
    <div
      ...
      style={{ border: `0.5px solid ${open ? "#2a3550" : "#1e2535"}`, background: "#0d1117" }}
    >
      {/* Header */}
      <button
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center gap-4 px-5 py-4 text-left hover:bg-[#111827] transition-colors"
      >
        <div
        ...
        <div className="hidden sm:flex items-center gap-3 flex-shrink-0">
          <div className="w-24 h-1.5 rounded-full bg-[#1e2535]">
      ...
      {/* Body */}
      {open && (
        <div className="px-5 pb-5 flex flex-col gap-4 border-t border-[#1e2535]">
          <div className="pt-4">
            <p className="text-gray-400 text-sm leading-relaxed">{section.feedback}</p>
          </div>
          <div className="flex flex-col gap-2">
            <p className="text-[10px] font-bold uppercase tracking-widest text-gray-600">Suggestions</p>
            {section.suggestions.map((sug, i) => (
              <div key={i} className="flex items-start gap-2.5">
                ...
                <p className="text-gray-400 text-sm">{sug}</p>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between mb-1">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-1">Section Analysis</p>
          <h3 className="text-white font-bold text-base">Resume Section Breakdown</h3>
        </div>
      </div>
