  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center justify-between text-xs">
        ...
        <span className="text-[var(--text-primary)] font-bold">{solved}<span className="text-[var(--text-muted)]">/{total}</span></span>
      </div>
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[var(--text-primary)] font-bold text-sm">LeetCode</p>
            <p className="text-[var(--text-muted)] text-xs">@{data.username}</p>
          </div>
        </div>
        <span
      ...
      <div className="grid grid-cols-2 gap-3 pt-1">
        <div className="rounded-xl p-3" style={{ background: "var(--bg-elevated)", border: "0.5px solid var(--border)" }}>
          <p className="text-[10px] uppercase tracking-widest text-[var(--text-muted)] mb-1">Acceptance Rate</p>
          <p className="text-[var(--text-primary)] text-lg font-extrabold">{data.acceptanceRate}%</p>
        </div>
        <div className="rounded-xl p-3" style={{ background: "var(--bg-elevated)", border: "0.5px solid var(--border)" }}>
          <p className="text-[10px] uppercase tracking-widest text-[var(--text-muted)] mb-1">Global Rank</p>
          <p className="text-[var(--text-primary)] text-lg font-extrabold">#{data.ranking.toLocaleString()}</p>
        </div>
      </div>
    </div>
  );
