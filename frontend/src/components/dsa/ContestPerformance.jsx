...
export default function ContestPerformance({ data }) {
  return (
    ...
    >
      <p className="text-[11px] font-semibold uppercase tracking-widest text-[var(--text-muted)]">Contest Performance</p>
          ...
          {(data?.recent ?? []).map((c) => (
            <div key={c.name} className="flex items-center justify-between p-2.5 rounded-lg" style={{ background: "var(--bg-elevated)" }}>
              <div>
                ...
                <p className="text-[var(--text-muted)] text-xs">{c.date} · Rank #{c.rank}</p>
              </div>
