  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4">
        ...
        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} style={{ color: "var(--brand-secondary)" }}>
      ...
      <div className="flex flex-col gap-3">
        {GOALS.map((g) => {
          ...
          return (
            <div
              key={g.id}
              className="flex items-start gap-3 p-3 rounded-xl cursor-pointer hover:bg-[var(--bg-hover)] transition-colors"
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[var(--text-primary)] text-sm font-semibold leading-snug">{g.title}</p>
                <p className="text-[var(--text-muted)] text-xs mt-0.5">{g.when}</p>
              </div>
              <span
